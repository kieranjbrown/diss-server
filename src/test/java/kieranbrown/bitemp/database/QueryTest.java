...
class QueryTest {
    ...
    void datesAreFormattedCorrectlyForSQL() {
        ...
        query.setFilters(List.of(
                new SingleQueryFilter(new Tuple3<>("valid_time_start", QueryEquality.EQUALS, LocalDate.of(2020, 2, 20)))
        ));
        assertThat(query.build()).isEqualTo("SELECT * from reporting.trade_data where valid_time_start = '2020-02-20'");

        query.setFilters(List.of(
                new SingleQueryFilter(new Tuple3<>("system_time_start", QueryEquality.EQUALS, new Date(2020, 1, 20, 13, 43, 0)))
        ));
        assertThat(query.build()).isEqualTo("SELECT * from reporting.trade_data where system_time_start = '2020-01-20 13:43:00.000000'");

        query.setFilters(List.of(
                new SingleQueryFilter(new Tuple3<>("system_time_start", QueryEquality.EQUALS, new Date(2020, 1, 20, 0, 0, 0)))
        ));
    ...
    void stringIsCorrectlyFormattedForSql() {
        final Query<Trade> query = new Query<>(QueryType.SELECT, Trade.class);
        query.setFilters(List.of(
                new SingleQueryFilter(new Tuple3<>("stock", QueryEquality.EQUALS, "AMZN"))
        ));
    ...
    void uuidIsCorrectlyFormatted() {
        ...
        query.setFilters(List.of(
                new SingleQueryFilter(new Tuple3<>("id", QueryEquality.EQUALS, uuid))
        ));
    ...
    void bigDecimalIsCorrectlyFormatted() {
        final Query<Trade> query = new Query<>(QueryType.SELECT, Trade.class);
        query.setFilters(List.of(
                new SingleQueryFilter(new Tuple3<>("price", QueryEquality.EQUALS, new BigDecimal("123.45")))
        ));
